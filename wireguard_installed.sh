...
for package in "${sw[@]}"
do
    if ! rpm -qa |grep -oE "^$package" |grep -v "lib" &> /dev/null
    then
        yum install $package -y &> /dev/null && echo "$package 安装完成"
    else
        echo "$package 存在无需安装"
    fi
done
...
start_menu(){
    ...
    echo "6. 增加用户"
    echo "0. 退出脚本"
    echo "服务器内核版本为$(uname -r), 如何内核版本小于4.0建议安装之前先升级下内核（注意：升级内核可能给系统的稳定性带来影响） "
